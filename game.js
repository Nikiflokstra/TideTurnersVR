...
AFRAME.registerComponent("game-manager", {
  init: function () {
    ...
    // NEW: cache begin button + ambience + controllers
    ...
    // start by clicking the panel or the button (desktop)
    this.startPanel.addEventListener("click", () => this.startLevel(0));
    if (this.startBtn)
      this.startBtn.addEventListener("click", () => this.startLevel(0));

    // also allow VR trigger to start while the startPanel is visible
    const tryStart = () => {
      if (
        this.i === -1 &&
        this.startPanel &&
        this.startPanel.getAttribute("visible")
      ) {
        this.startLevel(0);
      }
    };
    if (this.leftCtrl) this.leftCtrl.addEventListener("triggerdown", tryStart);
    if (this.rightCtrl)
      this.rightCtrl.addEventListener("triggerdown", tryStart);
...
startLevel: function (idx) {
  ...
  // ✅ show hands + net only after starting
