...
const MODEL_META = {
  // TRASH (collectable)
  bottle: { model: "#mdlBottle", scale: "0.4 0.4 0.4", collect: true },
  can: { model: "#mdlCan", scale: "0.5 0.5 0.5", collect: true }, // smaller soda can
  bag: {
    model: "#mdlBag",
    scale: "0.8 0.8 0.8",
    collect: true,
...
AFRAME.registerComponent("simple-trash-spawner", {
  ...
  init: function () {
    ...
    const nPlants = Math.max(6, Math.round(total * 1)); // 60% plants
    const nFish = Math.max(2, Math.round(total * 0.9)); // 25% fish
    const nJelly = Math.max(1, Math.round(total * 0.5)); // rest jelly
  ...
  spawnModel: function (key, opts) {
    ...
    if (key === "plant") y = 0.15 + Math.random() * -6; // ~0.15–0.40
