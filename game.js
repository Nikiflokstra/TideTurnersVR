...
AFRAME.registerComponent("simple-trash-spawner", {
  ...
  init: function () {
    ...
    const nPlants = Math.max(6, Math.round(total * 5)); // 100% plants (your latest tweak)
    const nFish = Math.max(2, Math.round(total * 2.5));
    const nJelly = Math.max(1, Math.round(total * 0.9));
  ...
  spawnModel: function (key, opts) {
    ...
    if (key === "plant") y = 0.15 + Math.random() * -6; // ~0.15–0.40 (kept as you had)
...
AFRAME.registerComponent("game-manager", {
  ...
  // >>> warning helpers (ADDED) <<<
  startWarning: function (L) {
    if (this._warnOn) return;
    this._warnOn = true;
    ...
    this._warnInterval = setInterval(() => {
      flip = !flip;
      const c = flip ? "#9c0b0b" : this._baseSky; // dark red flash
      this.sky.setAttribute("color", c);
      this.scene.setAttribute(
        "fog",
        `type: exponential; color: ${c}; density: ${this._baseFog}`
      );
      if (this.vrHud)
        this.vrHud.setAttribute("text", "color", flip ? "#ff6666" : "#ffffff");
    }, 250);
  },
  ...
  stopWarning: function () {
    ...
    const L = this.levels[this.i] || { sky: "#6cacbb", fog: 0.045 };
    this.sky.setAttribute("color", L.sky);
    this.scene.setAttribute(
      "fog",
      `type: exponential; color: ${L.sky}; density: ${L.fog}`
    );
    if (this.vrHud) this.vrHud.setAttribute("text", "color", "#ffffff");
  },
  // <<< end warning helpers >>>
  ...
  startLevel: function (idx) {
    ...
    // ensure warning off at level start (ADDED)
    if (this.stopWarning) this.stopWarning();
  ...
  tick: function (t, dt) {
    ...
    // start red flash at <= 10s (ADDED)
    if (this.timer <= 10 && !this._warnOn) {
      this.startWarning(this.levels[this.i]);
    }
